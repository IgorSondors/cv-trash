...
def generateData (numberOfClassEl, numberOfClasses):
    ...
    for classNum in range(numberOfClasses):
        ...
        centerX, centerY = random.random()*5.0, random.random()*5.0#на участке от 0 до 5 по каждой из осей выбирается местоположение мат ожидания гауссиана
        #Choose numberOfClassEl random nodes with RMS=0.5
        for rowNum in range(numberOfClassEl):
            data.append([ [random.gauss(centerX,0.5), random.gauss(centerY,0.5)], classNum])#среднеквадратичное отклонение 0.5
    return data
